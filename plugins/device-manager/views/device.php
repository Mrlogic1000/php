<div class="table-responsive">
    ...
    <table id="table" class="table table-striped ">
        ...
        <tr>
            ...
            <th>STATUS</th>
            <th>
                <button type="button" class="btn btn-primary" onclick="Device.display('new')">
                    Add New
                </button>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                ...
                                <button id="<?= $device->id ?>" class="edit btn btn-info" onclick="Device.row('<?= $device->id ?>','device-row',event)">
                                    <i class="fa-solid fa-pen-to-square"></i>

                                </button>
                                <?php if (user_can('delete_device')) : ?>
                                    <button id="<?= $device->id ?>" onclick="Device(<?= $device->id ?>,'device-delete',event)" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal">
...
<script>
    ...
    var Device = new CRUD(url, 'device');




    function createDevice(data, event) {
        event.preventDefault();
        Device.create(data)

    }

    function updateDevice(data, event) {
        event.preventDefault();
        Device.update(data)
    }
</script>