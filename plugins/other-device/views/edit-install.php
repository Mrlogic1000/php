      ...
      <select name="device_id" class="form-select form-select-sm mb-3" aria-label=".form-select-sm example">
        <?php if (!empty($devices)) : ?>
          ...
          <?php foreach ($devices as $device) : ?>
            <option <?= $install->device_id==$device->id  ? ' selected ' : '' ?> <?= old_selected('device_id', $device->id) ?> value="<?= $device->id ?>"><?= $device->name ?></option>
      ...
      <select name="outlet_id" class="form-select form-select-sm mb-3" aria-label=".form-select-sm example">
        <?php if (!empty($outlets)) : ?>
          ...
          <?php foreach ($outlets as $outlet) : ?>
            <option <?= $install->outlet_id==$outlet->id  ? ' selected ' : '' ?> <?= old_selected('outlet_id', $outlet->id) ?> value="<?= $outlet->id ?>"><?= $outlet->outlet ?></option>
